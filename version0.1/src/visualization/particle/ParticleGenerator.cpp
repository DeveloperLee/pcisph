...
ParticleGenerator::Boundary ParticleGenerator::generateBoundaryMesh(const Mesh &mesh, float particleRadius, int cells) {
    ...
    float density = 1.f / (M_PI * pow2(particleRadius));

    //PRINT("Generating surface particles ...");
    ...
    //PRINT("Generating initial point distribution ...");
    Boundary result;
    float totalArea = 0.f;
    ...
    //PRINT("Relaxing point distribution ...");
    ...
    for (int iteration = 0; iteration < 10; ++iteration) {
        ...
        for (size_t i = 0; i < result.positions.size(); ++i) {
            grid.lookup(result.positions[i], radius, [&] (size_t j) {
                ...
                if (r2 < radius2) {
                    r *= (1.f / std::sqrt(r2));
                    float weight = 0.002f * pow3(1.f - r2 / radius2); // TODO use a proper kernel
...
// Generate the initial box layout particle positions based on the setting file.
ParticleGenerator::Volume ParticleGenerator::generateVolumeBox(const Box3f &box, float particleRadius) {
    
    Vector3f extents = box.extents();
    int nx = std::ceil(extents.x() / (2.f * particleRadius)); // How many particles can fit in x-direction
    int ny = std::ceil(extents.y() / (2.f * particleRadius)); // How many particles can fit in y-direction
    int nz = std::ceil(extents.z() / (2.f * particleRadius)); // How many particles can fit in z-direction
    Vector3f o = box.min + Vector3f(particleRadius);  //Origin
    Vector3f d = extents.cwiseQuotient(Vector3f(nx, ny, nz)); // Unit offset vector
    Volume result;
    for (int x = 0; x < nx; ++x) {
        for (int y = 0; y < ny; ++y) {
            for (int z = 0; z < nz; ++z) {
...
// Generate the intial sphere layout particle positions based on the setting file.
