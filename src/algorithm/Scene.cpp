...
static std::string vectorToString(const std::vector<T> &items) {
    ...
    for (const auto &item : items) {
        result += "\n  " + StringUtils::indent(item.toString());
    }
    result += items.empty() ? "]" : "\n]";
    return result;
}
...
std::string Scene::toString() const {
    return tfm::format(
        ...
        "  cameraKeyframes = %s\n"
        "]",
        StringUtils::indent(settings.json().dump()),
        StringUtils::indent(camera.toString()),
        StringUtils::indent(world.toString()),
        StringUtils::indent(vectorToString(boxes)),
        StringUtils::indent(vectorToString(spheres)),
        StringUtils::indent(vectorToString(meshes)),
        StringUtils::indent(vectorToString(cameraKeyframes))
    );
}
